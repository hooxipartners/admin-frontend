...
export const BasicInfoTab = ({ data }: BasicInfoTabProps) => {
  ...
  const charges = data.accountList.filter(a => a.accountInfoTypeName === 'CHARGE')
          ...
          {etas && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex">
                  <div className="bg-[#f8fafc] border border-[#e4e7ec] rounded-l-[10px] px-3 py-2 text-xs text-[#637083] min-w-[40px] flex items-center justify-center">
                    ID
                  </div>
                  <Input
                    value={etas.loginId || ''}
                    readOnly={!isEditing}
                    className="bg-white border border-[#e4e7ec] rounded-r-[10px] rounded-l-none border-l-0 px-3 py-2 text-sm flex-1"
                    style={{ fontFamily: '"Inter-Regular", sans-serif' }}
                  />
                </div>
                {/* PW는 API에 없으므로 생략 */}
