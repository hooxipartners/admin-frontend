...
const FileCheckIcon = () => (
  <svg width="92" height="68" viewBox="0 0 92 68" fill="none" xmlns="http://www.w3.org/2000/svg">
    <mask id="path-1-inside-1_484_16792" fill="white">
      <path d="M0 0H92V68H0V0Z"/>
    </mask>
    <path d="M0 0H92V68H0V0Z" fill="white"/>
    <path d="M92 68V67H0V68V69H92V68Z" fill="#E4E7EC" mask="url(#path-1-inside-1_484_16792)"/>
    <path d="M36.375 34.4582L40.1111 38.1943C40.3259 38.4091 40.6741 38.4091 40.8889 38.1943L43.25 35.8332M49.6667 29.4165L46 33.0832M41.4167 33.9998L45.6111 38.1943C45.8259 38.4091 46.1741 38.4091 46.3889 38.1943L55.1667 29.4165" stroke="#10B978" stroke-width="1.5" stroke-linecap="round"/>
  </svg>
);
const FileErrorIcon = () => (
...
export const OperationInfoTab = () => {
  ...
  return (
    <div className="w-full min-h-screen pt-4">
      {/* 헤더 */}
      <div className="flex justify-between items-center h-12 border-[#e4e7ec] py-3 mb-6">
        <div className="flex items-center gap-2 ">
          <span
            className="text-[#141c25] font-medium text-[16px] leading-6"
        ...
        <div className="flex items-center gap-4">
          <span className="text-[#637083] text-xs">최근 업데이트 일시<br/>{lastUpdated}</span>
          <button className="flex items-center gap-2 py-2 px-5 rounded-[10px] border border-[#e4e7ec] bg-white shadow text-[#344051] text-sm font-medium">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><g clipPath="url(#clip0_770_5826)"><path d="M5.56476 17.1395C2.10975 15.0177 0.631928 10.6321 2.26422 6.78667C4.06251 2.55016 8.95468 0.573595 13.1912 2.37189C17.4277 4.17018 19.4043 9.06234 17.606 13.2988C16.9033 14.9542 15.7282 16.2646 14.3045 17.1395M18.3335 17.5001H14.6669C14.3907 17.5001 14.1669 17.2762 14.1669 17.0001V13.3334M10.0002 18.3418L10.0085 18.3325" stroke="#344051" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></g><defs><clipPath id="clip0_770_5826"><rect width="20" height="20" fill="white"/></clipPath></defs></svg>
            목록 업데이트
          </button>
          <button className="flex items-center gap-2 py-2 px-5 rounded-[10px] bg-[#0166ff] text-white text-sm font-medium">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 10H10M15 10H10M10 10V5M10 10V15" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            운행 추가
          </button>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center w-full gap-4 mb-6">
        <div className="flex items-center gap-2">
          <div className="flex items-center h-10 px-3 py-2 bg-white rounded-[10px] border border-[#e4e7ec]">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.99961 3H19.9997C20.552 3 20.9997 3.44764 20.9997 3.99987L20.9999 5.58569C21 5.85097 20.8946 6.10538 20.707 6.29295L14.2925 12.7071C14.105 12.8946 13.9996 13.149 13.9996 13.4142L13.9996 19.7192C13.9996 20.3698 13.3882 20.8472 12.7571 20.6894L10.7571 20.1894C10.3119 20.0781 9.99961 19.6781 9.99961 19.2192L9.99961 13.4142C9.99961 13.149 9.89425 12.8946 9.70672 12.7071L3.2925 6.29289C3.10496 6.10536 2.99961 5.851 2.99961 5.58579V4C2.99961 3.44772 3.44732 3 3.99961 3Z" stroke="#637083" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </div>
          <Select options={ACCOUNT_INFO_TYPE_OPTIONS.map(({ value, label }) => ({ value, label }))} placeholder="데이터유형" className="min-w-[120px]" />
          <div className="w-60 flex items-center">
            <Input className="flex-1 border border-[#e4e7ec] rounded-[10px] px-3 py-2 text-base h-10" placeholder="차량번호" />
          </div>
          <Button className="h-10 min-w-[80px] px-5 py-2 rounded-[10px] bg-[#141c25] text-white text-sm font-medium">검색</Button>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-[#637083]">Rows per page</span>
          <Select options={LIMIT_OPTIONS.map(opt => ({ label: opt.label, value: String(opt.value) }))} value={limit} onValueChange={setLimit} simple className="w-20" />
        </div>
      </div>
      {/* 테이블 */}
      <div className="w-full overflow-x-auto rounded-lg border border-[#e4e7ec] mb-6">
        <div className="flex bg-[#f2f4f7] min-w-[1200px]">
          {/* 컬럼 헤더 */}
          {tableColumns.map((column, idx) => (
            <div key={column.key} className={column.className + (idx === tableColumns.length - 1 ? ' border-r-0' : '')} style={{ ...(idx === 0 ? { borderTopLeftRadius: 12 } : {}), ...(idx === tableColumns.length - 1 ? { borderTopRightRadius: 12 } : {}) }}>
              <span className="text-xs font-medium text-[#344051] leading-5 whitespace-nowrap font-inter text-[14px] leading-5" style={{ fontFamily: 'Inter, sans-serif' }}>{column.label}</span>
            </div>
          ))}
        </div>
        {/* 테이블 본문 */}
        {rows.map((row, index) => (
          <div key={row.id} className={`flex bg-white hover:bg-gray-50 transition-colors min-w-[1200px] ${index !== rows.length - 1 ? 'border-b border-[#e4e7ec]' : ''}`} style={{ minHeight: '68px' }}>
            {/* 운행정보 데이터 */}
            <div className="flex-[2] min-w-[240px] flex items-center gap-2 px-4 py-2.5 border-r border-[#e4e7ec]">
              {/* PDF 아이콘 (32x34) */}
              <svg width="32" height="34" viewBox="0 0 32 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M5.84668 0.5H18.2568L29.0391 11.1377V30.5996C29.0391 32.2034 27.7453 33.5 26.1543 33.5H5.84668C4.25569 33.5 2.96191 32.2034 2.96191 30.5996V3.40039C2.96191 1.79662 4.25569 0.5 5.84668 0.5Z" fill="white" stroke="#E4E7EC"/>
                <path d="M10.1247 26.2144V21.1234H12.0338C12.4249 21.1234 12.753 21.1964 13.0182 21.3422C13.285 21.488 13.4863 21.6886 13.6222 21.9438C13.7598 22.1973 13.8285 22.4857 13.8285 22.8088C13.8285 23.1353 13.7598 23.4253 13.6222 23.6788C13.4847 23.9324 13.2816 24.1321 13.0132 24.2779C12.7447 24.4221 12.4141 24.4942 12.0214 24.4942H10.7561V23.736H11.8971C12.1258 23.736 12.313 23.6962 12.4589 23.6167C12.6047 23.5372 12.7124 23.4278 12.782 23.2886C12.8533 23.1494 12.8889 22.9895 12.8889 22.8088C12.8889 22.6282 12.8533 22.4691 12.782 22.3315C12.7124 22.194 12.6039 22.0871 12.4564 22.0109C12.3105 21.933 12.1224 21.894 11.8921 21.894H11.0469V26.2144H10.1247ZM16.3547 26.2144H14.6296V21.1234H16.3895C16.895 21.1234 17.3291 21.2254 17.6921 21.4292C18.0567 21.6314 18.3367 21.9222 18.5323 22.3017C18.7278 22.6812 18.8256 23.1353 18.8256 23.6639C18.8256 24.1942 18.727 24.65 18.5298 25.0311C18.3342 25.4123 18.0517 25.7048 17.6821 25.9086C17.3142 26.1124 16.8718 26.2144 16.3547 26.2144ZM15.5518 25.4164H16.31C16.6646 25.4164 16.9604 25.3518 17.1974 25.2225C17.4344 25.0916 17.6125 24.8969 17.7318 24.6384C17.8512 24.3782 17.9108 24.0534 17.9108 23.6639C17.9108 23.2745 17.8512 22.9513 17.7318 22.6945C17.6125 22.4359 17.436 22.2429 17.2024 22.1153C16.9704 21.986 16.682 21.9214 16.3373 21.9214H15.5518V25.4164ZM19.7018 26.2144V21.1234H22.9632V21.8965H20.6241V23.2786H22.7395V24.0517H20.6241V26.2144H19.7018Z" fill="#344051"/>
                <path d="M18.4619 6.72528V0L29.5388 10.9286H22.7223C20.3693 10.9286 18.4619 9.04669 18.4619 6.72528Z" fill="url(#paint0_radial_770_11398)"/>
                <defs>
                  <radialGradient id="paint0_radial_770_11398" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(24.0004 2.73214) rotate(90) scale(8.19643 8.30769)"><stop stopColor="#E4E7EC"/><stop offset="1" stopColor="#CED2DA"/></radialGradient>
                </defs>
              </svg>
              <span className="truncate text-[#141c25] text-sm font-medium">{row.fileName}</span>
            </div>
            {/* 데이터 범위 */}
            <div className="flex-1 min-w-[140px] flex items-center px-4 py-2.5 border-r border-[#e4e7ec] text-[#141c25] text-sm font-medium">{row.range}</div>
            {/* 데이터 유형 */}
            <div className="flex-1 min-w-[180px] flex items-center gap-2 px-4 py-2.5 border-r border-[#e4e7ec]">
            ...
            {/* 업로드상태 */}
            <div className="flex-1 min-w-[160px] flex items-center px-4 py-2.5 border-r border-[#e4e7ec]">
              <div className="w-full h-2 rounded-full bg-[#f2f4f7] flex items-center">
                <div className={`h-2 rounded-full ${row.upload === 100 ? 'bg-[#0166ff] w-full' : 'bg-[#0166ff]'} `} style={{ width: row.upload + '%' }} />
              </div>
            </div>
            {/* 파일등록일 */}
            <div className="flex-1 min-w-[120px] flex items-center px-4 py-2.5 border-r border-[#e4e7ec] text-[#141c25] text-sm font-medium">{row.date}</div>
            {/* 파일상태 */}
            <div className="flex-1 min-w-[80px] flex items-center justify-center px-4 py-2.5 border-r border-[#e4e7ec]">
              {row.status === 'check' && <FileCheckIcon />}
              {row.status === 'x' && <FileErrorIcon />}
              {row.status === 'loading' && <FileWaitIcon />}
            </div>
            {/* 비고(휴지통) */}
            <div className="flex-1 min-w-[80px] flex items-center justify-center px-4 py-2.5">
              <TrashIcon />
            </div>
          </div>
        ))}
